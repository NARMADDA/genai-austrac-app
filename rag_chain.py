...
# ✅ Cached WCS Client using Streamlit secrets
@st.cache_resource
def get_weaviate_client():
    return weaviate.Client(
        ...
        additional_headers={
            "X-OpenAI-Api-Key": st.secrets["AZURE_OPENAI_API_KEY"]
        }
    )

client = get_weaviate_client()

# 🔍 Query Weaviate for relevant AUSTRAC rules
def query_weaviate(query_text):
    try:
        ...
        # 🔁 Fallback to BM25 if near_text fails
        if not docs:
            response = client.query.get("AUSTRACRule", ["title", "section", "link"]) \
